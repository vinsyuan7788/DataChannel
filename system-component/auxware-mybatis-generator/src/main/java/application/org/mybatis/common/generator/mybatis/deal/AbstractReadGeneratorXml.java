	...
	public void initServiceImplGenerator() throws IOException {
		for (Element context : contextList) {
			...
			for (TableTag tableTag : tableTagList) {
				...
				resultList.add("    		params = GsonUtils.GSON.fromJson(GsonUtils.GSON.toJson(" + toLowerCase(domainObjectName) + "), Map.class); ");
				resultList.add("    	}");
				resultList.add("");
	...
	public void initControllerGenerator(String jsonScheamName) throws IOException {
		for (Element context : contextList) {
			...
			for (TableTag tableTag : tableTagList) {			
				...
				resultList.add("		String checkSchema = FileRedisCacheUtil.getFileContent(stringRedisCache,RedisKeyPrefixUtils." + GeneratorConstants.REDIS_KEY_PREFIX_FILE_FILE_KEY_PREFIX + ", fileDirectory, RedisKeyPrefixUtils." + jsonScheamName + ");");
				resultList.add("		// 从待校验的json字符串解析出实际业务内容的key");
				resultList.add("		String jsonKey = RedisKeyPrefixUtils." + GeneratorConstants.REDIS_KEY_PREFIX_FILE_JSON_DATE_KEY + ";");
				...
				resultList.add("			" + toLowerCase(domainObjectName) + ".set" + toUpperCase(jsonbFields.get(0)) + "(result.get(RedisKeyPrefixUtils." + GeneratorConstants.REDIS_KEY_PREFIX_FILE_JSON_DATE_KEY + "));");
				resultList.add("    		" + toLowerCase(domainObjectName) + "Service.insertSelective(" + toLowerCase(domainObjectName) + ");");
				resultList.add("    	    data.put(\"code\", \"0\");");
				resultList.add("    	    data.put(\"msg\", \"添加成功\");");
				resultList.add("            return GsonUtils.GSON.toJson(data);");
				resultList.add("        } else {");
				...
				resultList.add("		this.checkAdd(" + toLowerCase(domainObjectName) + ");");
				resultList.add("		String fileDirectory = InitializationUtil.getRootPath() + filePath.getFileRelativePath();");
				resultList.add("		result = JsonSchemaValidator.validateJsonByFgeByJsonString(" + toLowerCase(domainObjectName) + ".get" + toUpperCase(jsonbFields.get(0)) + "().toString(), FileRedisCacheUtil.getFileContent(stringRedisCache,RedisKeyPrefixUtils." + GeneratorConstants.REDIS_KEY_PREFIX_FILE_FILE_KEY_PREFIX + ", fileDirectory, RedisKeyPrefixUtils." + jsonScheamName + "), RedisKeyPrefixUtils." + GeneratorConstants.REDIS_KEY_PREFIX_FILE_JSON_DATE_KEY + ");");
				resultList.add("		if ((boolean) result.get(\"success\")) {");
				resultList.add("			" + toLowerCase(domainObjectName) + ".set" + toUpperCase(jsonbFields.get(0)) + "(result.get(RedisKeyPrefixUtils." + GeneratorConstants.REDIS_KEY_PREFIX_FILE_JSON_DATE_KEY + "));");
	...
	public void addRedisKeyPrefix(String jsonScheamName) throws IOException {
		...
		if (!redisKeyPrefixFile.exists()) {
			...
			*/
			...
			resultList.add(" *  </p>");
			resultList.add(" *	");
			resultList.add(" * 	@ClassName: RedisKeyPrefixUtils");
